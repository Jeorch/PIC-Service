...
trait RetrievalData {
    ...
    implicit val d2m : DBObject => Map[String, JsValue] = { obj =>
        ...
        Map(
            ...
            "sales" -> toJson(obj.getAs[Number]("sales").map (x => x).getOrElse(throw new Exception("product without sales value"))),
            "units" -> toJson(obj.getAs[Number]("units").map (x => x).getOrElse(throw new Exception("product without sales units"))),
